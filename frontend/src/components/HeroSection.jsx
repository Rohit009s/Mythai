...
export default function HeroSection({ onContinue }) {
  ...
  const totalSections = 3; // 3 sections: HORIZON, COSMOS, SPIRIT
  ...
  useEffect(() => {
    ...
    const createStarField = () => {
      const { current: refs } = threeRefs;
      const starCount = 10000; // Increased from 5000 to 10000
        ...
        const material = new THREE.ShaderMaterial({
          ...
          vertexShader: `
            ...
            void main() {
              ...
              // Very slow rotation based on depth (reduced from 0.05 to 0.02)
              float angle = time * 0.02 * (1.0 - depth * 0.2);
              mat2 rot = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
              pos.xy = rot * pos.xy;
              
              // Add slow drifting motion
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const totalProgress = progress * (totalSections - 1); // Normalize to 0-2 range
      const sectionProgress = totalProgress % 1;
      
      // Define camera positions for each section
      const cameraPositions = [
        { x: 0, y: 30, z: 100 },     // Section 0 - HORIZON (starting position)
        { x: 0, y: 40, z: -50 },     // Section 1 - COSMOS
        { x: 0, y: 50, z: -700 }     // Section 2 - SPIRIT
      ];
      ...
      // Auto-continue to deity selector after completing all sections
      if (progress > 0.95 && onContinue) {
       ...
       {[...Array(3)].map((_, i) => {
          ...
          const subtitles = [
            {
              ...
              line2: 'we bridge the divine and the earthly'
            },
            {
          ...
          return (
            <section key={i} className="content-section">
              ...
              {i === 2 && onContinue && (
                <button className="continue-btn-hero" onClick={onContinue}>
